...
from coin_dbase import select_data, insert_data, delete_record, modify_record, insert_user, select_user
...
def create_currency_record():
    if not request.json or not all(k in request.json for k in ['city', 'lat', 'lng']):
        return jsonify({'error':'the currency record needs to have a city, lat & long'}), 400
    new_record = {
        'id': request.json['city'],
